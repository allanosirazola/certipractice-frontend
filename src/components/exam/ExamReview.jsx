 ...
 const loadOptionsWithRetry = async (questionId, retries = 3) => {
    ...
    return (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div className="bg-white dark:bg-gray-800 rounded-lg p-8 text-center">
          ...
          <p className="text-gray-600 dark:text-gray-300">{t('examReview.loading')}</p>
        </div>
      </div>
    );
  }
  ...
  if (!exam || !currentQuestion) {
    return (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div className="bg-white dark:bg-gray-800 rounded-lg p-8 max-w-md w-full mx-4 text-center">
          <p className="text-gray-600 dark:text-gray-300">No hay preguntas para revisar con el filtro seleccionado.</p>
          <button
            onClick={onClose}
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white dark:bg-gray-800 rounded-lg max-w-6xl w-full mx-4 max-h-[95vh] overflow-hidden flex flex-col">
        
        {/* Header */}
        <div className="flex justify-between items-center p-6 border-b bg-gray-50 dark:bg-gray-900">
          <div>
            <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-100">{t('examReview.title2')}</h2>
            <p className="text-gray-600 dark:text-gray-300">{exam.certification} - {exam.provider}</p>
          ...
          <div className="max-w-4xl mx-auto">
            ...
            <div className="flex justify-between items-center mb-6 p-4 bg-gray-50 dark:bg-gray-900 rounded-lg">
              <div className="flex items-center gap-4">
                ...
                <span className="bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-400 px-2 py-1 rounded text-xs">
                  {currentQuestion.category}
                </span>
                <span className="bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-200 px-2 py-1 rounded text-xs">
                  {currentQuestion.difficulty}
                </span>
                {currentQuestion.isMultipleChoice && (
                  <span className="bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 px-2 py-1 rounded text-xs">
            ...
            {/* Pregunta */}
            <div className="mb-8">
              <h3 className="text-xl font-semibold text-gray-800 dark:text-gray-100 mb-4 leading-relaxed">
          ...
          >
            ...
            {questionDetails[currentQuestion.id] && (
              <div className="bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-800 rounded-lg p-6">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <h4 className="font-semibold text-blue-800 dark:text-blue-300 mb-3">{t('exam.explanation')}</h4>
            ...
            {loadingDetails && (
              <div className="bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg p-6 text-center">
                ...
                <p className="text-gray-600 dark:text-gray-300 text-sm">{t('examReview.loadingExplanation')}</p>
              </div>
            )}
          </div>
        </div>

        {/* Footer con navegación y descarga PDF */}
        <div className="border-t p-4 bg-gray-50 dark:bg-gray-900">
          <div className="flex justify-between items-center flex-wrap gap-3">
            <div className="text-sm text-gray-500 dark:text-gray-400">
              {filteredQuestions.length} {t('examReview.filterAll').toLowerCase()} · {filter}
            </div>
            ...
            <div className="flex gap-3 items-center">
              ...
              <button
                ...
                disabled={generatingPdf || !results}
                className="flex items-center gap-2 px-5 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50 font-medium text-sm shadow"
              >
                {generatingPdf ? (
                  <>
                    <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />
                    {i18n.language.startsWith('es') ? 'Generando…' : 'Generating…'}
                  </>
                ) : (
                  <>
                    ...
                    {i18n.language.startsWith('es') ? 'Descargar Informe PDF' : 'Download PDF Report'}
                  </>
                )}
              </button>
          ...
          <p className="text-xs text-gray-400 mt-2 text-right">
            {i18n.language.startsWith('es')
              ? '💼 El informe PDF es un documento profesional para presentar a tu empresa.'
              : '💼 The PDF report is a professional document to present to your employer.'}
          </p>
        </div>
      </div>
    </div>
